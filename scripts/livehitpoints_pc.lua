...
local function onFavoredClassBonusSelect_new(aSelection, rFavoredClassBonusSelect, ...)
	...
	for k, v in ipairs(aSelection) do
		if v == Interface.getString('char_value_favoredclasshpbonus') then
			DB.setValue(
				rFavoredClassBonusSelect.nodeChar,
				'livehp.misc',
				'number',
				DB.getValue(rFavoredClassBonusSelect.nodeChar, 'livehp.misc', 0) + 1
			)
			setHpTotal(ActorManager.resolveActor(rFavoredClassBonusSelect.nodeChar))
			local sMsg =
				string.format(Interface.getString('char_message_favoredclasshpadd'), DB.getValue(rFavoredClassBonusSelect.nodeChar, 'name', ''))
			ChatManager.SystemMessage(sMsg)
			aSelection[k] = nil
		end
	end
